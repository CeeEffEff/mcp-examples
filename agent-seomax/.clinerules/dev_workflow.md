...
8.  **`new_task` for Handover**: At logical stages of completion, use the `new_task` tool to hand over work to another agent with full context.
9.  **Repeat**.

## Agent Handover Rule

**CRITICAL**: At logical stages of completion (e.g., after completing a subtask, finishing a significant implementation phase, or reaching a natural stopping point), you MUST use the `new_task` tool to hand over to another agent. This ensures:
- Continuous work without context degradation
- Fresh perspective on the next steps
- Efficient token usage and context management
- Clear transition points in the development workflow

**When to use new_task**:
- After completing a subtask and marking it as done
- After implementing a significant feature component
- When reaching a natural breakpoint in work
- Before switching to a different area of the codebase
- After updating rules or documentation
